...
class TimeInstant(TimeInstantJulianDateAndUnixTimestamp, TimeInstantFormatString):
  ...
  def to_struct_time(self, time_zone: TimeZone | None = None, date_cls: type[JulGregBaseDate] = GregorianDate):
    if time_zone == None:
      ...
      date = date_cls(year, month, day)
      current_tz_offset = self.current_tz_offset(time_zone, date_cls = date_cls)
      return struct_time(
        (year, month, day, hour, minute, second, date.iso_day_of_week() - 1, date.ordinal_date(), current_tz_offset != time_zone.initial_utc_offset),
        {
          'tm_zone': 'NULL',
